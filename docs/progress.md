...
最終更新: 2026-03-26（バグ修正: OAuth失効・タイトル省略・日付ずれ・削除検索）
...
- [x] HTMLメール・添付ファイル認識 — 「PDFが添付されています」と通知（2026-03-26 完了: Phase 12で実装）
...
> ~~月末業務予告通知~~ → 繰り返し予定として手動登録で対応（ユーザーが「毎月X日」に自由にカスタマイズ可）
> ~~金曜週次総括~~ → 繰り返し予定として手動登録で対応（summary_report インテント活用）

#### 🟢 Medium（会話品質・UX）
- [x] ペルソナ応答パターンの拡充 — 凛の応答パターンをsystem_prompt.pyに追記（励まし・体調確認・労いなど感情的なサポート）（2026-03-26 完了）
- [x] HTMLメール・添付ファイル認識 — メール本文のHTMLタグ除去 + 「PDFが添付されています」と通知（2026-03-26 完了）
- [x] タスク進捗ステータス拡充 — `in_progress`（作業中）の明示的利用（現状は pending/done の2値のみ実質使用）（2026-03-26 完了）
...
### 2026-03-26（Phase 12 🟢 Medium 3件完了）
- **ペルソナ応答パターン拡充**: `system_prompt.py` に【感情的サポート】セクション追加（励まし・労い・体調気遣い・達成感・週明け・週末前・繁忙期の7パターン）
- **HTMLメール・添付ファイル認識**: `gmail_service.py` に `_html_to_text()` / `_extract_attachments()` 追加 — HTML本文をプレーンテキスト変換、添付ファイル名・サイズを本文末尾に付記
- **タスク進捗ステータス拡充**: `task_service.py` に `start_task()` 追加（pending→in_progress）、`format_tasks_for_display` に🔧バッジ追加、`secretary.py` で「着手/開始/取りかかる」キーワード検出→ルーティング
...
- **日付1日ずれバグ修正**: LLMが「明日」を「3/28」等の具体日付に変換してしまう問題 → プロンプトに「計算・変換しない」と強調、LLM出力を無視して元メッセージから再抽出するフォールバックを追加
- **「明後日の予定を削除して」が動かないバグ修正**: `_handle_schedule_delete()` がキーワード検索のみ対応で日付表現に非対応だった → 日付表現を `_resolve_date()` で解決してその日の予定を取得するロジック追加、キーワード＋日付の複合指定にも対応、`_match_event_date()` ヘルパー追加
