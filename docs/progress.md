...
### Gmail連携（Phase 7）
- [ ] OAuthスコープ追加（gmail.readonly + gmail.send + gmail.compose）→ トークン再取得
- [ ] gmail_service.py — Gmail API接続・メール取得・下書き・送信
- [ ] メールトリアージ — ルールベース除外 + LLM 3段階分類（要返信/要確認/スキップ）
- [ ] mail_check インテント — 「メール確認」で重要メール一覧表示
- [ ] mail_detail インテント — 「メール1の詳細」で本文要約表示
- [ ] mail_draft インテント — 「メール1に下書き。〇〇と伝えて」→ 確認 → Gmail下書き保存
- [ ] mail_reply インテント — 「メール1に返信して。〇〇と伝えて」→ 確認 → 直接送信
- [ ] mail_drafts インテント — 「下書き一覧」で凛が作成した下書きを表示
- [ ] mail_send インテント — 「下書き1を送信して」で保存済み下書きを送信
- [ ] 朝ブリーフィングに未読重要メール件数・概要を追加
- [ ] MailFilterRule モデル — ユーザーフィードバックで仕分けルールを学習・蓄積
- [ ] PIIフィルタ: メール本文のGemini送信時はPIIフィルタ必須（Ollama優先）

### ヘルプ・リッチメニュー拡張（Phase 7）
- [ ] help インテント — 「ヘルプ」「使い方」「何ができる？」で機能一覧を分かりやすく表示
- [ ] リッチメニュー9ボタン化（3x3）— メール確認・下書き一覧・ヘルプを追加
